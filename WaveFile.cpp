				 ...
				 )
{
	...
	for (cT = 0; cT < cbDataIn; cT++)
	{
		...
		// *((BYTE*)mmioinfoIn.pchNext) = *((BYTE*)mmioinfoIn.pchNext) + 1;
	}
				   ...
				   )
{
	...
	*phmmioOut = mmioOpen((LPSTR)pszFileName, NULL,
				  ...
				  )
{
	...
	*cbActualWrite = 0;
	...
	{       
		...
		*((BYTE*)pmmioinfoOut->pchNext) = *((BYTE*)pmmioinfoOut->pchNext) + *((BYTE*)pbSrc+cT);
		(*cbActualWrite)++;
	}
